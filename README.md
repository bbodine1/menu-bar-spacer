...
- Launch-at-login toggle using `SMAppService.mainApp.register()` on macOS 13+ with legacy `SMLoginItemSetEnabled` fallback.
- Basic unit tests covering creation, resizing, persistence, deletion, and login-item toggling signals.

## Development
The project is configured as a Swift Package targeting macOS 12 and later. Open the folder in Xcode to run and debug the menu bar app. The executable target is named `MenuBarSpacer` and uses an `@main` SwiftUI `App` entry point.
